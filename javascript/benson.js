document.addEventListener('DOMContentLoaded', function () {
    // Filter data for Benson Memorial
    const bensonData = wasteData.filter(item => item.Building.toLowerCase() === 'benson center');

    // Extract unique dates from the filtered data
    const uniqueDates = [...new Set(bensonData.map(item => item.Date.split('T')[0]))];
        ...
        return new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                    title: {
                        display: true,
                        text: 'Waste Data Visualization for Benson Center (For each Month)',
                        font: {
                            size: 24
                        }
                    },
    ...
    datePicker.addEventListener('change', (event) => {
        ...
        const filteredData = bensonData.filter(d => d.Date.startsWith(date));
